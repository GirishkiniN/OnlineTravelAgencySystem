<!DOCTYPE html>
<head>
    <title>ADMIN | BUS DRIVERS DETAILS</title>
     ...
     <div class="full_sht">
        ...
        <div class="cntnt" >
            ...
            <div class="display scroll">
                <br>
              <h1 style="color:white;"><center><b>BUS DRIVER DETAILS</b></center></h1>
          <br>
          <center><input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for Driver ID.." title="Type in a name"></center>
                    <br>
        <table id="myTable">
        <tr>
            <th>DRIVER ID</th>
        ...
        <?php
                ...
                $sql = "SELECT * FROM busdriverdetails";
                $result = $conn->query($sql);
                $result = $conn->query($sql);
                $rows = mysqli_num_rows($result);
                if($rows==0)
                {
                    echo"<tr><td colspan='8'>NO BUS DRIVERS AVAILABLE.....</td></tr>";
                }
                else
                {
            ...
            ?> 
            <tr>
                <td><?php echo $rows['d_u_name'];?></td>
                <td><?php echo $rows['date_join'];?></td>
                <td><img src="../StaffPortals/busStaff/image/<?php echo $rows['d_photo'];?>" width="35px" height="35px"></td>
                <td><?php echo $rows['d_name'];?></td>
                <td><?php echo $rows['d_age'];?></td>
                <td><?php echo $rows['d_address'];?></td>
                <td><?php echo $rows['d_mno'];?></td>
                <td><?php echo $rows['d_busNo'];?></td>
            </tr>
        <?php  } } ?>    
    </table>
